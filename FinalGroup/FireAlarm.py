...
class FireAlarm:
    ...
    def __init__(self):
        ...
        self._manualB = Button(16, "Manual Button", buttonhandler=None)
        self._disarmB = Button(10, "Disarm Button", buttonhandler=None)
        self._redL = Light(2,'Red LED')
        self._blueL = Light(0,'Blue LED')
        self._evacuateP = NeoPixel(pin=17, numleds=10, brightness=1)
        self._labS = DigitalSensor(28)
        self._breakroomS = DigitalSensor(28)
        self._alertBuz = PassiveBuzzer(pin=19)
    ...
    def stateDo(self, state):
        ...
        if state == 0:
            ...
            if self._breakroomS.tripped():
                self._model.gotoState(3)
        

        # transition to state 4 after action completed
        elif state == 1:
